/**********************************************************
 * WHOLESALE ORDER MANAGEMENT SYSTEM - NEW FUNCTIONS
 * Add these functions to Code.gs after the existing Orders section
 **********************************************************/
...
function getMasterOrders() {
  try {
    ...
    if (!sheet) {
      sheet = createMasterOrdersSheet_(ss);
    }
...
/**
 * Save (create or update) a master order
 */
function saveMasterOrder(orderData) {
  try {
    ...
    // Generate ID if new order
    if (!orderData.id) {
      var year = new Date().getFullYear();
      var count = data.length;
      orderData.id = 'MO-' + year + '-' + String(count).padStart(3, '0');
    }
...
function getShipments(orderID) {
  try {
    ...
    if (!sheet) {
      sheet = createShipmentsSheet_(ss);
    }
...
function saveShipment(shipmentData) {
  try {
    ...
    if (!shipmentData.id) {
      ...
      var shipmentNum = String(data.length).padStart(2, '0');
      shipmentData.id = 'SH-' + orderNum + '-' + shipmentNum;
    }
...
function getPriceHistory() {
  try {
    ...
    if (!sheet) {
      sheet = createPriceHistorySheet_(ss);
    }
...
/**
 * Sync COA index from Google Drive
 */
...
/**
 * Get order financials (commitment, fulfilled, paid, outstanding)
 */
 ...
 * 2. Scroll to the end of Code.gs (after the testOrdersAPI function around line 3005)
 * 3. Copy and paste all the functions above
 * 4. Save and deploy
 **********************************************************/
