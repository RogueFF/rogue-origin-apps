...
const S = {
  es: {
    ...
    binsPrefilled: 'Ya viene {n} — cámbialo si la traila viene incompleta',
    logLoad: 'Anotar carga',
    lotAtDoor: '¿De qué lote viene esta carga?',
    lotAtDoorHint: 'No hay zona abierta aquí. Pregúntale a los cortadores de qué lote es — si no, las cajas no se suman a ningún lote.',
    lotAtDoorNone: 'Sin lote — anotar de todos modos',
    lotAtDoorOpen: 'abierto',
    lotAtDoorBad: 'Ese lote no es de esta zona o ya pasó mucho tiempo. Escoge uno de la lista.',
    lotChosen: '✅ Anotado al lote que escogiste: <strong>{lot}</strong>.',
    ...
    testRealRow: '⚠ Modo de prueba: esta es una bolsa real y no se puede cambiar desde aquí. Quita el modo de prueba primero.',
    dayEnd: 'Fin del día',
    ...
    // — cerrar un lote —
    finishLot: '✓ Terminé con este lote',
    finishLotHelp: 'Cuando el rack esté vacío. Sale de la lista de bajada; no cambia bolsas ni inventario, y se puede reabrir.',
    confirmFinish: '¿Marcar {lot} como terminado?\n\n{n} etiquetas impresas. Sale de la lista de bajada — se puede reabrir.',
    finishSection: '¿Terminó de bajar un lote?',
    finishSectionHelp: 'Márcalo terminado y sale de la lista. No imprime nada ni cambia el inventario.',
    markFinished: 'Terminado',
    finishedLots: 'Lotes terminados ({n})',
    finishedOn: 'terminado {date} · {n} bolsas',
    reopenLot: 'Reabrir',
    lotFinished: '{lot} terminado — {n} bolsas.',
    ...
    editNote: 'Editar', noteEdited: 'editada {d}', noteUpdated: 'Nota actualizada.', noteNotFound: 'No se encontró esa nota en esta bolsa.',
    noteSaved: 'Nota guardada.', noteEmpty: 'Escribe algo antes de guardar.',
    noteNext: 'Nota para la próxima etiqueta', printTagNote: 'IMPRIMIR + NOTA',
    noteNextHint: 'Solo si algo está mal con esta bolsa. Escríbela antes de IMPRIMIR ETIQUETA y se guarda en esa etiqueta. Una tanda no lleva nota.',
    noteSavedOn: '✓ Nota guardada en <strong>#&nbsp;{id}</strong>',
    approx: 'aprox.',
    ...
    // — dónde se secó y dónde se guarda (bahías 1-12 o el Supermarket) —
    secLocation: 'Ubicación', locDried: 'Se secó en', locStored: 'Guardada en',
    lotHungInOne: 'el lote se colgó en la bahía {list}', lotHungIn: 'el lote se colgó en las bahías {list}',
    storageNone: 'sin registrar', storedSince: 'desde {d}',
    storeChange: 'Cambiar dónde está guardada', storeSave: 'Guardar',
    storageSaved: 'Guardada en {where}.', storageCleared: 'Ubicación borrada.',
    storageInvalid: 'La ubicación debe ser una bahía de {min} a {max}, o el Supermarket.',
    storeOpened: 'Esta bolsa ya se abrió — ya no está guardada.',
    storeVoided: 'Esta etiqueta está anulada — no hay bolsa que guardar.',
    storageField: 'Dónde se guardan', storageHint: '¿A dónde van las bolsas? "Todavía no" está bien',
    storageHintLast: 'La última fue {where}', storageNotYet: 'Todavía no',
    storageHintStale: 'La última fue {where}, otro día — elige de nuevo',
    storedInWhere: 'Se guardan en {where}', storageNotSet: 'Sin lugar para guardar',
  ...
  en: {
    ...
    binsPrefilled: 'Pre-filled {n} — change it if the trailer came in short',
    logLoad: 'Log load',
    lotAtDoor: 'Which lot did this trailer come from?',
    lotAtDoorHint: 'No zone is open here. Ask the cutters which lot it is — otherwise these bins count toward no lot at all.',
    lotAtDoorNone: 'No lot — log it anyway',
    lotAtDoorOpen: 'open',
    lotAtDoorBad: 'That lot is not in this zone, or it is too old. Pick one from the list.',
    lotChosen: '✅ Logged to the lot you chose: <strong>{lot}</strong>.',
    ...
    testBand: "⚠ Test mode",
    testRealRow: "⚠ Test mode: this is a real bag, and nothing here can change it. Turn test mode off first.",
    dayEnd: 'End of day',
    ...
    finishLot: '✓ Finished with this lot',
    finishLotHelp: 'When the rack is empty. It leaves the takedown list; no sack or inventory changes, and it can be reopened.',
    confirmFinish: 'Mark {lot} finished?\n\n{n} tags printed. It leaves the takedown list — you can reopen it.',
    finishSection: 'Done taking a lot down?',
    finishSectionHelp: 'Mark it finished so it leaves the list. Nothing prints and inventory does not change.',
    markFinished: 'Finished',
    finishedLots: 'Finished lots ({n})',
    finishedOn: 'finished {date} · {n} sacks',
    reopenLot: 'Reopen',
    lotFinished: '{lot} marked finished — {n} sacks.',
    ...
    editNote: 'Edit', noteEdited: 'edited {d}', noteUpdated: 'Note updated.', noteNotFound: 'That note was not found on this sack.',
    noteSaved: 'Note saved.', noteEmpty: 'Write something before saving.',
    noteNext: 'Note for the next tag', printTagNote: 'PRINT TAG + NOTE',
    noteNextHint: 'Only if something is wrong with this bag. Type it before PRINT TAG and it is saved on that tag. A batch does not take a note.',
    noteSavedOn: '✓ Note saved on <strong>#&nbsp;{id}</strong>',
    approx: 'approx.',
    ...
    secLocation: 'Location', locDried: 'Dried in', locStored: 'Stored in',
    lotHungInOne: 'lot hung in bay {list}', lotHungIn: 'lot hung in bays {list}',
    storageNone: 'not recorded', storedSince: 'since {d}',
    storeChange: 'Change where it is stored', storeSave: 'Save',
    storageSaved: 'Stored in {where}.', storageCleared: 'Storage cleared.',
    storageInvalid: 'Storage must be a bay from {min} to {max}, or the Supermarket.',
    storeOpened: 'This sack is already opened — it is not in storage any more.',
    storeVoided: 'This tag is voided — there is no sack to store.',
    storageField: 'Where they are stored', storageHint: 'Where are the sacks going? "Not yet" is fine',
    storageHintLast: 'Last was {where}', storageNotYet: 'Not yet',
    storageHintStale: 'Last was {where}, on another day — pick again',
    storedInWhere: 'Stored in {where}', storageNotSet: 'No storage set',
