...
function updateImagePreview(url) {
  ...
  if (url && url.trim()) {
    ...
    if (sanitizedUrl) {
      preview.innerHTML = '<img src="' + esc(sanitizedUrl) + '" onerror="this.style.display=\'none\'"><button type="button" onclick="clearImageUrl()">✕ Clear</button>';
...
function updateAllText() {
  try {
    ...
    if (langBtn) langBtn.textContent = currentLang === 'en' ? '<i class="ph-duotone ph-translate"></i> Español' : '<i class="ph-duotone ph-translate"></i> English';
...
function renderGrid() {
  ...
  filtered.forEach(function(c) {
    html += '<div class="card">' +
      ...
      '<div class="card-preview-img">' + (c.picture && sanitizeImageUrl(c.picture) ? '<img src="' + esc(sanitizeImageUrl(c.picture)) + '" onerror="this.outerHTML=\'📷\'">' : '📷') + '</div>' +
...
function cardHtml(c, color, side, size) {
  var qrSize = (size === 'full' || size === 'shop-letter' || size === 'shop-portrait') ? '400x400' : '200x200';
  ...
  if (size === 'shop') {
    return '<div class="kanban ' + color + '"><div class="kanban-border"></div><div class="kanban-inner"></div>' +
      ...
      '<div class="kanban-right" style="justify-content:center;align-items:center"><div class="kanban-img">' + (c.picture && sanitizeImageUrl(c.picture) ? '<img src="' + esc(sanitizeImageUrl(c.picture)) + '" onerror="this.outerHTML=\'📷\'">' : '📷') + '</div>' +
  ...
  return '<div class="kanban ' + color + '"><div class="kanban-border"></div><div class="kanban-inner"></div>' +
    ...
    '<div class="kanban-right"><div class="kanban-img">' + (c.picture && sanitizeImageUrl(c.picture) ? '<img src="' + esc(sanitizeImageUrl(c.picture)) + '" onerror="this.outerHTML=\'📷\'">' : '📷') + '</div>' +
...
function updatePrintCount() {
  ...
  var cardsPerPage = (size === 'full' || size === 'shop-letter' || size === 'shop-portrait') ? 1 : 2;
  // Shop-letter and shop-portrait always print front only with neutral color (1 page per card)
  var pages = (size === 'shop-letter' || size === 'shop-portrait') ? selectedIds.length : Math.ceil((selectedIds.length * sidesCount * colorsCount) / cardsPerPage);

  document.getElementById('printCount').textContent = selectedIds.length;
  document.getElementById('printPages').textContent = pages;

  var sizeInfo = {
    'card': '6"×4" cards, 2 per page',
    'tag': '4"×2" tags, compact layout',
    'shop': '6"×3" shop labels, simplified',
    'shop-portrait': '6"×4" neutral shop card, front only (portrait)',
    'shop-letter': '10"×7.5" neutral shop card, front only (landscape)',
    'full': '7.5"×5" full sheet, 1 per page'
  };
...
function updatePreviewSizeInfo() {
  ...
  var sizeInfo = {
    'card': '6"×4" cards, 2 per page (green + red)',
    'tag': '4"×2" compact tags, 2 per page',
    'shop': '6"×3" shop labels, simplified layout',
    'shop-portrait': '6"×4" neutral shop card, front only (portrait)',
    'shop-letter': '10"×7.5" neutral shop card, front only (landscape)',
    'full': '7.5"×5" full sheet, 1 card per page'
  };
...
function getPrintCSS(size) {
  ...
  if (size === 'tag') {
    ...
    return '@page{size:letter landscape;margin:0.25in}*{box-sizing:border-box;margin:0;padding:0}' +
      'body{font-family:system-ui,sans-serif;background:#faf8f5;-webkit-print-color-adjust:exact;print-color-adjust:exact}' +
      '.page{page-break-after:always;width:10.5in;height:8in;margin:0;display:flex;justify-content:center;align-items:center;box-sizing:border-box}' +
      '.page:last-child{page-break-after:auto}' +
      '.page-inner{display:flex;flex-direction:column;align-items:center;justify-content:center;width:100%;height:100%}' +
      '.kanban{width:10in;height:7.5in;padding:20px;border-radius:12px;position:relative;flex-shrink:0;-webkit-print-color-adjust:exact;print-color-adjust:exact}' +
      ...
      '.no-print button{padding:10px 20px;border:none;border-radius:6px;cursor:pointer;margin-right:8px}' +
      '@media print{.no-print{display:none}.print-wrapper{padding-top:0 !important}body{background:#fff}}';
  } else if (size === 'shop-portrait') {
    // 6" x 4" portrait shop card - neutral colors, maximum visibility
    return '@page{size:letter portrait;margin:0.5in}*{box-sizing:border-box;margin:0;padding:0}' +
      'body{font-family:system-ui,sans-serif;background:#faf8f5;-webkit-print-color-adjust:exact;print-color-adjust:exact}' +
      '.page{page-break-after:always;width:8.5in;height:11in;margin:0;display:flex;justify-content:center;align-items:center;box-sizing:border-box}' +
      '.page:last-child{page-break-after:auto}' +
      '.page-inner{display:flex;flex-direction:column;align-items:center;justify-content:center;width:100%;height:100%}' +
      '.kanban{width:6in;height:4in;padding:12px;border-radius:10px;position:relative;flex-shrink:0;-webkit-print-color-adjust:exact;print-color-adjust:exact}' +
      '.kanban{background:#e8e4de !important;color:#2d3a2e !important}' +
      '.kanban-border{position:absolute;inset:0;border:5px solid #8f9263;border-radius:10px}' +
      '.kanban-inner{position:absolute;inset:14px;border:4px solid #8f9263;border-radius:8px}' +
      '.kanban-content{height:100%;display:flex;flex-direction:column;padding:20px 24px}' +
      '.kanban-header{border-bottom:3px solid currentColor;padding-bottom:8px;margin-bottom:6px}' +
      '.kanban-header h3{text-transform:uppercase;margin:0;font-weight:700;letter-spacing:1px;font-size:26px}' +
      '.kanban-loc{text-align:right;font-weight:700;font-size:18px;margin-bottom:4px}' +
      '.kanban-body{flex:1;display:flex;gap:20px;align-items:center}' +
      '.kanban-left{flex:1;display:flex;flex-direction:column;justify-content:center;gap:8px}' +
      '.kanban-field{margin-bottom:0}' +
      '.kanban-field small{text-transform:uppercase;opacity:0.8;display:block;font-weight:600;font-size:12px;margin-bottom:2px}' +
      '.kanban-field span{font-weight:700;font-size:22px}' +
      '.kanban-right{display:flex;flex-direction:column;align-items:flex-end;justify-content:center;gap:12px}' +
      '.kanban-img{background:#fff;border-radius:8px;border:4px solid #8f9263;display:flex;align-items:center;justify-content:center;overflow:hidden;width:180px;height:130px;-webkit-print-color-adjust:exact;print-color-adjust:exact}' +
      '.kanban-img img{width:100%;height:100%;object-fit:contain}' +
      '.kanban-price{background:#8f9263;border-radius:25px;font-weight:700;color:#faf8f5;padding:8px 32px;font-size:22px;-webkit-print-color-adjust:exact;print-color-adjust:exact}' +
      '.kanban-logo{position:absolute;border-radius:50%;bottom:22px;left:22px;width:50px;height:50px}' +
      '.kanban-back{height:100%;display:flex;flex-direction:column;align-items:center;justify-content:center}' +
      '.kanban-back h3{text-transform:uppercase;font-weight:700;letter-spacing:2px;font-size:42px;margin-bottom:28px}' +
      '.kanban-qr{background:#fff;border-radius:10px;border:4px solid #e4aa4f;display:flex;align-items:center;justify-content:center;width:200px;height:200px;-webkit-print-color-adjust:exact;print-color-adjust:exact}' +
      '.kanban-qr img{width:92%;height:92%}' +
      '.no-print{padding:12px;background:#1a1f16;color:#fff;position:fixed;top:0;left:0;right:0;z-index:100}' +
      '.no-print button{padding:10px 20px;border:none;border-radius:6px;cursor:pointer;margin-right:8px}' +
      '@media print{.no-print{display:none}.print-wrapper{padding-top:0 !important}body{background:#fff}}';
...
function printOneCard(card, size) {
  var allHtml = [];
  if (size === 'shop-letter' || size === 'shop-portrait') {
  ...
  var html = '<!DOCTYPE html><html><head><title>Kanban Cards</title><style>' +
    ...
    '<span style="margin-left:12px;color:#8f9263">' + allHtml.length + ' page(s) • ' + size.toUpperCase() + '</span></div>' +
...
var tutorialSteps = [
  // PART 1: Page Overview
  {
    ...
    tip: '⏱️ This takes about 2-3 minutes',
    position: 'center'
  },
  {
    target: '.header',
    ...
    tip: '👆 The header stays at the top so you can always access these buttons'
  },
  {
    ...
    tip: '🔄 Good habit: Refresh at the start of your shift to see the latest cards'
  },
  {
    ...
    tip: '🖨️ We\'ll use this at the end to print the card we create'
  },
  {
    ...
    tip: '✨ We\'ll click this soon to create your first card'
  },
  {
    target: '.search-bar',
  ...
  },
  {
    ...
    tip: '🟢 Green = In Stock | 🟡 Gold = Low/Reorder | 🟠 Orange = Critical'
  },
  {
    target: '.card:first-child .card-actions',
    title: '<i class="ph-duotone ph-cursor-click"></i> Card Action Buttons',
    text: 'Every card has these 4 buttons: 🛒 Mark for reorder, 👁️ Preview the printable card, ✏️ Edit the card, 🗑️ Delete the card.',
  ...
  // PART 2: Let's Create a Card
  {
    ...
    tip: '🎯 Click Next and we\'ll open the Add Card form',
    position: 'center',
    action: 'openModal'
  },
  {
    ...
    tip: '✏️ Go ahead and type in this box now!',
  ...
  },
  {
    ...
    tip: '🏪 This tells you where to buy it when you need to reorder'
  },
  {
    target: '#editQty',
    ...
    tip: '📦 Match this to your typical order size'
  },
  {
    target: '#editPrice',
    ...
    tip: '💰 This helps track costs and compare suppliers'
  },
  {
    target: '#editDelivery',
    ...
    tip: '🚚 Helps with planning when to reorder'
  },
  {
    target: '#editLoc',
    ...
    tip: '📍 Example: "Cleaning Rack > F-1" means Cleaning Rack, slot F-1'
  },
  {
    target: '#editUrl',
    ...
    tip: '🔗 The QR code lets you scan and go straight to the order page'
  },
  {
    target: '#autofillBtn',
    ...
    tip: '✨ This is a huge time-saver when adding lots of products'
  },
  {
    ...
    tip: '🖼️ Try dragging an image from Google Images or the product page'
  },
  {
    target: '#editSave',
    ...
    tip: '✅ Click Save now! (Don\'t worry, you can edit or delete it later)',
    action: 'waitForSave'
  },
  // PART 3: After Saving
  {
    ...
    tip: '🎉 Your card is in the system. Now let\'s print it!',
    position: 'center',
  ...
  },
  {
    ...
    tip: '☑️ Click a checkbox now to select a card!'
  },
  {
    target: '#printBtn',
    ...
    tip: '🖨️ The QR codes link directly to the product order page'
  },
  {
    target: null,
    ...
    tip: '🌟 Go forth and organize your supplies!',
    position: 'center',
    isLast: true
  }
];
...
function endTutorial(completed) {
  ...
  if (completed) {
    ...
    createConfetti();
    setTimeout(function() {
      toast('🎉 Tutorial complete! You\'re a Kanban pro now!', 'ok');
    }, 500);
  }
}
