...
test('customer dropdown has customers', async ({ page }) => {
  ...
  // Check if [CUSTOMER REDACTED] is in the list
  const has[CUSTOMER REDACTED] = options.some(opt => opt.includes('[CUSTOMER REDACTED]'));
  console.log('Has [CUSTOMER REDACTED]:', has[CUSTOMER REDACTED]);
  expect(has[CUSTOMER REDACTED]).toBe(true);
});
