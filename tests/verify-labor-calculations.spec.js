...
test('verify labor cost calculations with waterspider', async ({ page }) => {
  ...
  console.log(`  Total Tops: ${today.totalTops} lbs`);
  console.log(`  Total Smalls: ${today.totalSmalls} lbs`);
  console.log(`  Total Production: ${today.totalLbs} lbs`);
  ...
  console.log(`    Cost/lb: $${today.laborCost.toFixed(2)} / ${today.totalLbs} lbs (tops + smalls) = $${today.costPerLb.toFixed(2)}/lb`);

  // Check historical full day
  const dec28 = response.daily?.find(d => d.date === '2025-12-28');
  if (dec28) {
    console.log('\n\nDEC 28 (FULL DAY):');
    console.log(`  Total Tops: ${dec28.totalTops} lbs`);
    console.log(`  Total Smalls: ${dec28.totalSmalls} lbs`);
    console.log(`  Total Production: ${dec28.totalLbs} lbs`);
    ...
    console.log(`    Cost/lb: $${dec28.laborCost.toFixed(2)} / ${dec28.totalLbs} lbs (${dec28.totalTops} tops + ${dec28.totalSmalls} smalls) = $${dec28.costPerLb.toFixed(2)}/lb`);
  }
