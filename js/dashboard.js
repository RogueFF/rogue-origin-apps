    ...
    function initMuuriGrid() {
      ...
      setTimeout(function() {
        ...
        // Load saved layout after initial layout
        setTimeout(function() {
          loadLayout();
        }, 50);
      }, 150);
    ...
    function renderAll() {
      ...
      if (muuriGrid) {
        // Wait for charts/images to render, then recalculate layout
        setTimeout(function() {
          muuriGrid.refreshItems();
          muuriGrid.layout(true);  // Force synchronous layout

          // Additional pass to ensure everything settled
          setTimeout(function() {
            muuriGrid.layout(true);
          }, 100);
        }, 200);
    ...
    function renderHero() {
      ...
      animateCountUp(document.getElementById('heroProductionNumber'), tops, 1, 1200);

      // Subtitle with total and smalls breakdown
      document.getElementById('heroSubtitle').textContent = 'Total: ' + totalProduction.toFixed(1) + ' lbs (incl. ' + smalls.toFixed(1) + ' lbs smalls)';
      ...
      document.getElementById('heroProgressFill').style.width = Math.min(progressPercent, 100) + '%';
      document.getElementById('heroProgressText').textContent = progressPercent.toFixed(0) + '% of predicted tops';

      // Mini KPIs
      var crew = (t.trimmers || 0) + (t.buckers || 0) + (t.qc || 0) + (t.tzero || 0);
      document.getElementById('heroCrewValue').textContent = crew;

      document.getElementById('heroRateValue').textContent = rate.toFixed(2);

      // Display predicted tops outcome (using already calculated predictedTops from above)
      if (predictedTops > 0) {
        document.getElementById('heroTargetValue').textContent = predictedTops.toFixed(0) + ' lbs';
      } else {
        document.getElementById('heroTargetValue').textContent = '--';
      }

      // Bags done (if available from timer data)
      document.getElementById('heroBagsValue').textContent = t.bagsCompleted || '--';
    }
