...
// --staged: hash what git will actually commit, not what happens to be sitting
// in the working tree. See stagedContent below for why that distinction matters.
 ...
 */
/**
 * Files whose working copy differs from the index. Computed once; on a typical
 * commit this is a handful of files, so only those need a git round-trip.
 */
const dirtyFiles = (() => {
  if (!useStaged) return new Set();
  try {
...
/**
 * Contents as git will commit them.
 *
 * A pre-commit hook must hash the INDEX, not the working tree. Hashing the
 * working tree bakes uncommitted edits into the committed HTML, and the result
 * is worse than a wrong number: the page ships pointing at the hash of a file
 * that was never deployed, and when that edit is finally committed the file
 * takes on the hash the HTML already carries — so no cache bust ever fires.
 * The one change guaranteed to need busting is the one that silently does not.
 *
 * Caught by CI on 2026-08-13: scoreboard-v2.html carried timer.js?h=da4870be,
 * the hash of an uncommitted edit, while the deployed timer.js hashed 28d8d058.
 */
function readForHash(file) {
  const rel = relative(REPO_ROOT, file).split('\\').join('/');
  if (useStaged && dirtyFiles.has(rel)) {
    try {
      return execFileSync('git', ['show', `:${rel}`], {
        cwd: REPO_ROOT, encoding: 'utf8', maxBuffer: 1 << 26
      });
    } catch {
      // Not in the index (newly added, unstaged) — the working copy is what
      // will be committed if the author stages it, so fall back to that.
...
function contentHash(file) {
  const normalised = readForHash(file).replace(/\r\n/g, '\n');
