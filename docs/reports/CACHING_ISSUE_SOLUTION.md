...
export const API_URL = `https://script.google.com/macros/s/REDACTED-PRODUCTION-API-ID/exec?v=${API_VERSION}`;
```
...
export function getAPIURL() {
  return `https://script.google.com/macros/s/REDACTED-PRODUCTION-API-ID/exec?t=${Date.now()}`;
}
```
