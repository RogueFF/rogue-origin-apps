...
| H6 | ✅ **Resolved by deletion** — `ops-hub.html` (the legacy hub with the broken scoreboard iframe) was removed; all 14 app home buttons now point to `index.html`, the SW precache + `ops-hub.css` were cleaned up. | ~~`src/pages/ops-hub.html`~~ | Done. |
