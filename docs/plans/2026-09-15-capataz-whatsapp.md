...
`gsmSafe`/`smsSegments`/`MAX_SMS_SEGMENTS` exist only to keep an SMS inside GSM-7 160-char segments — meaningless for WhatsApp, which is UTF-8. Skipping them for `channel='whatsapp'` stops accented Spanish (é, í, ñ, ¿, ¡) from being stripped out of *relay-composed* replies (the model's own text, and any text sent through `sendToForeman`/`sms_send`). It does **not** touch the worker's own canned Spanish strings (`promptText`, `reminderText`, `helpText`, the hardcoded command replies in `processInbound` — "dia", "manana", "Cuantos", etc.) — those are authored unaccented in `harvest-hourly.js` on purpose (that's what made `smsSegments`' plain-length math correct for SMS) and stay that way; only the relay's replies gain accents back. This must land together with Task 7 (the relay's own unconditional accent-stripping) — landing only one side is wasted work, since whichever side still strips wins.
